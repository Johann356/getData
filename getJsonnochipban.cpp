...
string processIDCardJson() {
	...
	if (strVec.size() > 10 && strVec[strVec.size() - 1] == "1") {//港澳身份证
		passportInfo["hxcIDHKMNu"] = trim(strVec[10]);
		passportInfo["RenewalTimes"] = trim(strVec[11]);
	}
	else if (strVec.size() > 10 && strVec[strVec.size() - 1] == "2") {//外国人永久居留证
		passportInfo["FullName"] = trim(strVec[9]);
		passportInfo["SerialNum"] = trim(strVec[10]);
	}
